...
object FoldSuite extends SimpleTestSuite with HarikoChecker {
  ...
  test("Fold: Applicative") {
    ...
    // associativity:
    check(Property.forAll[(M[Boolean], M[Boolean], M[Boolean])] {
      case (x, y, z) =>
  ...
  test("Fold: CoflatMap") {
    ...
    // associativity:
    check(Property.forAll[(M[Boolean], M[Boolean] => Boolean, M[Boolean] => Boolean)] {
      case (fa, f, g) =>
  ...
  test("Fold: Comonad") {
    ...
    // right identity:
    check(Property.forAll[(M[Boolean], M[Boolean] => Boolean)] {
      case (fa, f) =>
